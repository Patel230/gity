 ...
 * same-origin Pages Functions (functions/api/*). PKCE binds the flow to this
 * browser session (state + code_verifier live in sessionStorage only).
 */

const AUTHORIZE_URL = "https://github.com/login/oauth/authorize";

/** True when the deployment serves the /api exchange (Cloudflare Pages). */
export function loginAvailable(): boolean {
  // Same-origin API — present on Cloudflare Pages, absent on plain static hosts.
  return true;
}
...
export async function finishLogin(search: string): Promise<OAuthTokens> {
  ...
  if (err)
    throw new Error(
      err === "access_denied" ? "You denied access on GitHub." : `GitHub refused authorization (${err}).`,
    );
