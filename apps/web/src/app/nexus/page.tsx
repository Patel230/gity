...
export default function NexusPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {view === "architecture" ? <ArchitectureView graph={graph} repos={filteredRepos} selectedNode={selectedNode} selectedEdge={selectedEdge} expandedOrganizations={expandedOrganizations} onSelectNode={selectNode} onSelectEdge={(edge) => { setSelectedEdgeId(edge.id); setSelectedNodeId(null); }} onToggleOrganization={toggleOrganization} /> : null}
      {view === "services" ? <ServicesView graph={graph} selectedNode={selectedNode} isLoading={serviceCandidates.isLoading} error={serviceCandidates.error} onSelectNode={selectNode} onPlayFlow={(serviceId) => { setFlowServiceId(serviceId); setView("flows"); }} /> : null}
      ...
      {view === "repositories" ? <RepositoriesView repos={filteredRepos} /> : null}
      ...
      {view === "changes" ? <ChangesView repos={filteredRepos} latestRunByRepo={latestRunByRepo} /> : null}
    </div>
  );
}
...
function EntityPanel({ node, edge, graph, onSelectNode, onPlayFlow = () => undefined }: { node: NexusNode | null; edge: NexusEdge | null; graph: NexusGraphSnapshot; onSelectNode: (node: NexusNode) => void; onPlayFlow?: () => void }) {
...
function ServicesView({ graph, selectedNode, isLoading, error, onSelectNode, onPlayFlow }: { graph: NexusGraphSnapshot; selectedNode: NexusNode | null; isLoading: boolean; error: Error | null; onSelectNode: (node: NexusNode) => void; onPlayFlow: (serviceId: string) => void }) {
  const services = graph.nodes.filter((node) => node.type === "service");
...
function RepositoriesView({ repos }: { repos: GithubRepo[] }) { return <Card accent={2}><CardHeader><CardTitle className="flex items-center gap-1.5"><Database className="size-3.5 text-[var(--primary)]" /> Repositories</CardTitle><CardDescription>Repositories remain the evidence boundary, not the whole system model.</CardDescription></CardHeader><CardContent><div className="mb-3 flex items-center justify-between rounded-md border border-border/70 bg-background/25 p-3"><p className="text-xs text-muted-foreground">The detailed repository map includes codebase shape, README, commits, current work, ownership, and verified cross-repository references.</p><Link href="/map" className="inline-flex shrink-0 items-center gap-1 text-xs text-[var(--primary)] hover:underline">Open repository map <ArrowUpRight className="size-3" /></Link></div><div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">{repos.slice(0, 30).map((repo) => <Link key={repo.fullName} href={`/map?repo=${encodeURIComponent(repo.fullName)}`} className="rounded-md border border-border/70 bg-background/25 p-2.5 transition hover:border-primary/60 hover:bg-accent"><div className="flex items-center justify-between gap-2"><span className="truncate font-mono text-xs font-medium">{repo.fullName}</span><CiBadge state={repo.ciState} /></div><p className="mt-1 truncate text-[10px] text-muted-foreground">{repo.description ?? "No description"}</p><p className="mt-2 text-[10px] text-muted-foreground">{repo.primaryLanguage ?? "Undeclared"} · pushed {timeAgo(repo.pushedAt)}</p></Link>)}</div>{repos.length > 30 ? <p className="mt-2 text-[10px] text-muted-foreground">+{repos.length - 30} more repositories · narrow the search to focus</p> : null}</CardContent></Card>; }
...
const TOUR_STEPS = [
  ...
  { title: "Inspect a repository", text: "Repository nodes link to the detailed Map dossier while remaining part of the larger system model." },
...
function DocsView() { return <div className="grid gap-4 lg:grid-cols-2"><Card accent={5}><CardHeader><CardTitle className="flex items-center gap-1.5"><FileText className="size-3.5 text-[var(--primary)]" /> Nexus documentation</CardTitle><CardDescription>Architecture decisions live beside the implementation so the model can evolve coherently.</CardDescription></CardHeader><CardContent className="space-y-2 text-xs text-muted-foreground"><p><strong className="text-foreground">Evidence first.</strong> Provider metadata and deterministic evidence are distinct from inferred conclusions.</p><p><strong className="text-foreground">Progressive disclosure.</strong> Organization and domain groups are expanded before services and interfaces.</p><p><strong className="text-foreground">Compatibility.</strong> `/map` remains the detailed repository surface while Nexus becomes the umbrella.</p><Link href="/map" className="inline-flex items-center gap-1 pt-2 text-[var(--primary)] hover:underline">Read the repository evidence surface <ArrowUpRight className="size-3" /></Link></CardContent></Card><PlaceholderView icon={FileText} title="Documentation context is next" description="Repository docs will become evidence attached to services and flows, not an absolute source of truth. Drift detection will compare documentation with detected architecture." items={["README and design-doc context", "Contract/documentation drift", "Source locator and confidence", "Reviewable architecture notes"]} /></div>; }
function ChangesView({ repos, latestRunByRepo }: { repos: GithubRepo[]; latestRunByRepo: Map<string, { workflowName: string; status: string; conclusion: string | null; updatedAt: string }> }) { const ordered = repos.slice().sort((a, b) => (b.pushedAt ?? "").localeCompare(a.pushedAt ?? "")).slice(0, 12); return <Card accent={6}><CardHeader><CardTitle className="flex items-center gap-1.5"><GitCommitHorizontal className="size-3.5 text-[var(--primary)]" /> Architectural changes</CardTitle><CardDescription>Recent repository and delivery signals are visible now; affected-service analysis will follow the service index.</CardDescription></CardHeader><CardContent><div className="space-y-1.5">{ordered.map((repo) => { const run = latestRunByRepo.get(repo.fullName); return <Link key={repo.fullName} href={`/map?repo=${encodeURIComponent(repo.fullName)}`} className="flex items-center gap-3 rounded-md border border-border/70 bg-background/25 px-3 py-2.5 hover:border-primary/60 hover:bg-accent"><span className="grid size-7 shrink-0 place-items-center rounded bg-accent text-[var(--primary)]"><GitBranch className="size-3.5" /></span><span className="min-w-0 flex-1"><span className="block truncate font-mono text-xs font-medium">{repo.fullName}</span><span className="mt-0.5 block truncate text-[10px] text-muted-foreground">pushed {timeAgo(repo.pushedAt)} · {repo.primaryLanguage ?? "language undeclared"}</span></span>{run ? <Badge variant={run.conclusion === "failure" ? "destructive" : run.status === "in_progress" ? "info" : "outline"}>{run.workflowName} · {run.conclusion ?? run.status}</Badge> : <Badge variant="outline">no Actions run</Badge>}<ArrowUpRight className="size-3 shrink-0 text-muted-foreground" /></Link>; })}{ordered.length === 0 ? <EmptyState title="No recent changes" hint="No repositories match this scope." /> : null}</div><p className="mt-3 text-[10px] text-muted-foreground">This view does not claim a service was affected until architecture evidence can support that conclusion.</p></CardContent></Card>; }
